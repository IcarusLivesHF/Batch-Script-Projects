...
for /l %%i in (0,%steps%,%tau%) do (
	set /a "phi=180*%%i/360",^
		   ...
		   "h=!degrees:x=%%i! * 10, s=10000, l=5000, %@hsl.rgb%"
	set "pre=!pre!"!r!;!g!;!b! !x! !y! !z!" "
)
...
(
	for /l %%# in (1,1,10000) do (
		...
		set /a "radx+=%radians:x=2%, rady+=%radians:x=2%, radz+=%radians:x=2%, %@3Dangles%"
		
		for %%p in (%pre%) do for /f "tokens=1-4 delims= " %%w in ("%%~p") do (
			
			set /a "x=%%x, y=%%y, z=%%z, %@ortho%, $x+=center_x, $y+=center_y"

			set "spiral=!spiral!%\e%[!$y!;!$x!H%\e%[48;2;%%wm%\e%[2X%\e%[B  "
